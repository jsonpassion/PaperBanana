...
### 2026-03-02: 모델 교체 및 안정성 개선

**모델 전면 교체** (gemini-3.1 시리즈 프리뷰 기간 만료, 할당량 0 이슈)
- 메인 모델: `gemini-3.1-pro-preview` → `gemini-2.5-pro`
- 이미지 모델: `gemini-3.1-flash-image-preview` → `gemini-2.0-flash-exp`
- lite 모델: `gemini-2.0-flash` → `gemini-2.5-flash` (Smart Input용)

**API 호출 안정성 개선**
- 할당량 0 (모델 폐기) 감지 시 즉시 실패 처리 — 불필요한 재시도 대기 제거
- 이미지 모델 감지: 모델 이름 기반 → 응답 데이터(inline_data) 기반으로 변경, 모델명 변경에도 안정적 동작
